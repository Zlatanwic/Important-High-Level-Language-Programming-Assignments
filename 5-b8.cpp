...
void calc_next(char d_cur[], char d_next[])
{
	...
	for (int i = 1; i <= N_NUM; i++) {
		if (int(d_cur[i]) != 0) {
			if ((int(d_cur[i]) - 48) % 2 == 0) {
				if (int(d_next[i]) == 0)
					d_next[i] = char(int(d_next[i]) + (int(d_cur[i]) - 48) / 2 + 48);
				else {
					d_next[i] = char(int(d_next[i]) - 48 + (int(d_cur[i]) - 48) / 2 + 48);
				}
			}
			else {
				if (int(d_next[i]) == 0)
					...
					d_next[i] = char(int(d_next[i]) - 48 + (int(d_cur[i]) - 48) / 2 + 48);
				if (int(d_next[i + 1]) == 0)
					d_next[i + 1] = char(int(d_next[i + 1]) + 5 + 48);
			}
		}
	}
}
		...
		if (pure_decimal[i] != '0') {
			for (int j = N_NUM; j > 0; j--) {
				if (power_table[i][j] != 0) {
					if (int(out[j]) == 0)
						out[j] = char(int(out[j]) + int(power_table[i][j]));
					else
						out[j] = char(int(out[j]) - 48 + int(power_table[i][j]));
					if ((int(out[j]) - 48) >= 10) {
						out[j] = char(int(out[j]) - 10);
						out[j - 1] = char(int(out[j - 1]) + 1);
					}
				}
			}
		}
...
void input_pure_decimal(char pure_decimal[])
{
	...
	while (1) {
		int rule_or_not = 1;
		for (int t = 0; t < N_NUM + 1; t++) {
			pure_decimal[t] = getchar();
			if (pure_decimal[t] == 10) {
				pure_decimal[t] = 0;
				break;
			}
		}
		...
		if (pure_decimal[0] != '.')
			...
			for (int m = 1; m < N_NUM; m++) {
				if (pure_decimal[m] != '1' && pure_decimal[m] != '0' && pure_decimal[m] != 0 && pure_decimal[m] != 10) {
					rule_or_not = 0;
				}
			}
			if (rule_or_not == 0)
				cout << "输入的不是0/1，请重输" << endl;
			else if (rule_or_not == 1)
				break;
		}
	}
	//cout << endl;
...
int main()
{
	...
	*/
	for (i = 1; i <= N_POW; i++) {
		power_table[i][0] = -i; //幂次 -1 ~ -52,在函数内打印负号
	}
	power_table[1][1] = char(5 + 48); // 2^-1 = 0.5，先置[1][1]为5
